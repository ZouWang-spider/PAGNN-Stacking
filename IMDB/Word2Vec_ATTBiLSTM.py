...
#词汇表大小
vocab_size=len(index_dict.keys())                                 #词汇表大小93597
print(vocab_size)
...
x=pad_sequences(maxlen=300,sequences=x,padding='post',value=0)            #填充大小为100
...
#标签类别数量
label_size=len(label_dictionary.keys())                                #标签大小2正面 反面 
...
print(train_x.shape)       #20000,300
print(train_y.shape)       #20000,2
...
model=Sequential()
model.add(Embedding(input_dim=n_symbols,output_dim=100, mask_zero=True,weights=[embedding_weights],input_length=300))
...
hist=model.fit(train_x,train_y,batch_size=32,epochs=10,verbose=2,validation_split=0.2)
