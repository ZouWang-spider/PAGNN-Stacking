...
rf_model.fit(meta_train,train_y2)
RF_pred=rf_model.predict(meta_test)
...
gbdt_model.fit(meta_train,train_y2)
GBDT_pred=gbdt_model.predict(meta_test)


adb_model = AdaBoostClassifier()                    #提升算法
adb_model.fit(meta_train,train_y2)
AdaBoost_pred=adb_model.predict(meta_test)

#5种模型预测并结果存储RF_pred,SVC_pred,KNN_pred,GBDT_pred,AdaBoost_pred
test_sets = np.zeros((meta_test.shape[0], 5))            #产生12400行5列的0矩阵
test_sets[:, 0] = RF_pred
test_sets[:, 1] = SVC_pred
test_sets[:, 2] = KNN_pred
test_sets[:, 3] = GBDT_pred
test_sets[:, 4] = AdaBoost_pred
#print(test_sets)                        (12400,5)

#新的数据集为test_sets(12400,5)  test_y2(12400,)
#将新的数据集划分(9920,5) (2480,5)   (9920,)  (2480,)
train_x3, test_x3, train_y3, test_y3 = train_test_split(test_sets,test_y2,test_size=0.2,random_state=42)
#使用决策树作为我们的元分类器
from sklearn.linear_model import LogisticRegression
lr_model = LogisticRegression()                         #决策树算法（CART决策树）
lr_model.fit(train_x3, train_y3)
LR_predict = lr_model.predict(test_x3)
print(LR_predict)
...
print('交叉验证结果:')
print(cross_val_score(lr_model,test_x3, test_y3,cv=3))

#衡量标准
from sklearn.metrics import classification_report
print('精度、召回率、F1值：')
print(classification_report(test_y3,LR_predict))
